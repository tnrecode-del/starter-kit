...
### Правила Изоляции Задач (Orchestrator V5)

Для корректной работы автономной кодогенерации и предотвращения конфликтов в монорепозитории введены строгие правила:

1. **Каждая задача — своя ветка**: Оркестратор всегда создает новую Git ветку вида `feature/task-{id}` от `main` перед выполнением задачи.
2. **Разрешение зависимостей**: Задачи не могут ветвиться друг от друга. Если Задача B зависит от Задачи A, Оркестратор ждет полного слияния (merge) ветки Задачи A в `main`, прежде чем начать работу над Задачей B. Это гарантирует актуальность контекста.
